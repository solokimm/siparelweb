if (typeof data != 'undefined') {
					...
					$.ajax({
						...
						success: function (response) {
							if (response.status == 'success') {
								...
								window.location.href = `${base_url}users-management`;
								blockUI.release();
							} else {
								swal.fire({
									icon: 'error',
	...
	$('a#editUser').click(function () {
		blockUI.block();
		$.ajax({
			url: `${base_url}users-management/userById`,
			method: 'POST',
			data: {
				id: user_id,
			},
			dataType: 'JSON',
			success: function (response) {
				$('input#edit_nip').val(response['nip']);
				...
				$('select#edit_role').val(response['role']).trigger('change');
	...
	let form_edit = (function () {
		...
		submitButton.click(function () {
			fv.validate().then(function (status) {
				if (status === 'Valid') {
					blockUI.block();
					$.ajax({
						url: `${base_url}users-management/edit`,
						type: 'POST',
						data: {
							id: user_id,
		...
		$('select#edit_role').on('change', function () {
			fv.revalidateField('edit_role');
		});
